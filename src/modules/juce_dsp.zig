pub const juce_module = JuceModule.init("juce_dsp", createModule);
...
fn createModule(ctx: JuceModule.BuildContext) *std.Build.Module {
    ...
    module.addCSourceFiles(.{
        .root = ctx.upstream.path("modules/juce_dsp"),
        .files = &.{ctx.builder.fmt("juce_dsp.{s}", .{if (is_darwin) "mm" else "cpp"})},
    });
    if (is_darwin) {
