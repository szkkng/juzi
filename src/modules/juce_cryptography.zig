pub const juce_module = JuceModule.init("juce_cryptography", createModule);
...
fn createModule(ctx: JuceModule.BuildContext) *std.Build.Module {
    ...
    module.addCSourceFiles(.{
        .root = ctx.upstream.path("modules/juce_cryptography"),
        .files = &.{ctx.builder.fmt("juce_cryptography.{s}", .{if (is_darwin) "mm" else "cpp"})},
    });
    if (is_darwin) {
